...
export default function AnalyticsPanel({ agentId }: { agentId: number }) {
  ...
  return (
    <Box>
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          ...
          <Stack direction="row" sx={{ alignItems: 'center' }} spacing={2}>
            <Box sx={{ flex: 1 }}>
              <LinearProgress variant="determinate" value={data.ai_handled_pct} sx={{ height: 12, borderRadius: 6 }} />
            </Box>
            <Typography sx={{ fontWeight: 800 }}>{data.ai_handled_pct}%</Typography>
          </Stack>
      ...
      <Card>
        <CardContent>
          <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 2 }}>Pesan masuk 7 hari terakhir</Typography>
          <Stack spacing={1}>
            {data.trend.map(t => (
              <Stack key={t.day} direction="row" sx={{ alignItems: 'center' }} spacing={1}>
                ...
                <Box sx={{ flex: 1, bgcolor: '#eceff1', borderRadius: 1, height: 22, position: 'relative' }}>
